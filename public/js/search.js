...
function applyFilters() {
    ...
    if (time === "afternoon") result = result.filter(c => hour(c) >= 12 && hour(c) <= 16);
...
function addToSchedule(c) {
  ...
  const hasConflict = schedule.some((item) => {
    ...
    console.log(`Before adding ${c.id}: availableSeats=${c.availableSeats}, availableWait=${c.availableWait}`);
    ...
    console.log(`After adding ${c.id}: availableSeats=${c.availableSeats}, availableWait=${c.availableWait}`);
    //lets users k ow
